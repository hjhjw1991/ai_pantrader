 ...
 *
 * 只做一件事：去掉结尾的"账户"二字，让 `卫星` 与 `卫星账户` 指向同一个账户。
 * 这个别名需求是真实的：示例 YAML 写"卫星账户"，而 ledger 契约的
 * paramPath 示例写"持仓.卫星.止损"，一份照文档抄下来的 YAML 必须读得出规则。
...
export const StrategyConfigSchema = z.looseObject({
  ...
  选股: z.looseObject({
    ...
    /**
     * 买点。触发价 = 昨收 × (1 + 相对昨收)，再按 不高于MA5 决定要不要用 MA5 压一道。
     *
     * 做成参数而不是写死，是因为这条规则**直接决定推荐能不能成交**：
     * 原先写死成 min(昨收, MA5)，而候选主要来自昨日涨停池 ——
     * 刚涨停的票 MA5 必然远在收盘价之下，次日通常还要高开，
     * 买点从生成的那一刻起就在市价下方 5~10%。
     * 实测 4 年回测（964 个交易日）21 条买入决策**成交 0 笔**，触发率 0%，
     * 于是台账永远攒不出样本，胜率也就无从谈起。
     *
     * 上下界 ±20%：超出这个范围的买点在 A 股 10% 涨跌停下没有意义，
     * 写错一个小数点就会让整批推荐要么必成交要么永不成交。
     */
    买点: z.looseObject({
