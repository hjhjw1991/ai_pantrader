 ...
 *   只做启动发酵+结构位 冷启动回放里冰点开仓最差、启动最好，于是只在启动 / 发酵开仓。
 *                    同样是样本内挑出来的，毕业只认实盘
 *   申万主线（三套）   主线不靠写死的必查链，由申万行业的涨停分布决定。单换主线识别器一套，
 *                    另两套叠在目前回放最好的两个组合上
 *
...
export const DEFAULT_VARIANTS: VariantDef[] = [
  ...
  { id: "cycle-strict+pricing", name: "只做启动发酵+结构位",
    slots: {
      择时器: { 用: "五段状态机", 参数: { 阶段档位: { 冰点: "防守", 启动: "进攻", 发酵: "进攻", 高潮: "防守", 退潮: "防守" } } },
      评估器: { 用: "结构位定价" },
    },
    note: "冰点 / 高潮也空仓，只在启动与发酵开仓。来自冷启动回放的分阶段结果（五段+结构位：启动 +0.40%/笔、" +
      "冰点 −0.67%/笔）—— 这是从样本内挑出来的假设，不能自证，只能靠实盘影子检验" },
  { id: "sw", name: "申万主线", slots: { 主线识别器: { 用: "申万聚集" } },
    note: "只换主线识别器：主线由申万行业的涨停聚集与高度龙头决定，不用写死的必查链（用户 2026-09-24 提出）" },
  { id: "sw+cycle+pricing", name: "申万主线+五段+结构位",
    slots: { 主线识别器: { 用: "申万聚集" }, 择时器: { 用: "五段状态机" }, 评估器: { 用: "结构位定价" } },
    note: "五段+结构位换上申万主线" },
  { id: "sw+cycle-strict+pricing", name: "申万主线+只做启动发酵+结构位",
    slots: {
      主线识别器: { 用: "申万聚集" },
      择时器: { 用: "五段状态机", 参数: { 阶段档位: { 冰点: "防守", 启动: "进攻", 发酵: "进攻", 高潮: "防守", 退潮: "防守" } } },
      评估器: { 用: "结构位定价" },
    },
    note: "只做启动发酵+结构位换上申万主线" },
];
