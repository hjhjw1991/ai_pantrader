...
只有一个主页：`/` 作战台。顶部是环境档位、情绪阶段、影子盘摘要；左栏是候选池（含目标价与盈亏比）、持仓动作、观察池建议，右栏是交互式 K 线（点任意「看图」换票，不刷新页面）。其余功能都是从右侧滑出的抽屉，数字键直达、Esc 关闭，抽屉地址可刷新可收藏：

| 键 | 路径 | 抽屉 |
|---|---|---|
| 1 | `/positions` | 持仓、盈亏、止损距离、成交回填、**账户管理** |
| 2 | `/watchpool` | 观察池：每只带触发价 + 止损价 + 一句话逻辑 |
| 3 | `/shadow` | 影子盘：各组合成绩、毕业进度、待批切换、回滚 |
| 4 | `/market` | 盘面原料：涨停池聚合、连板梯队、板块涨幅、龙虎榜 |
| 5 | `/ledger` | 预测台账与对账：胜率、偏差归因、参数回写建议 |
| 6 | `/lab` | 回测与 walk-forward |
| 7 | `/settings` | 数据源健康、缺口、调度状态、**策略管理**、参数面板、导入导出 |
