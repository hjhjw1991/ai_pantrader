...
- **影子盘与正式台账分开记、分开算**：每天 09:15 出正式计划后，6 套槽位组合（baseline、结构位定价、五段状态机、五段+结构位、五段高潮进攻+结构位、只做启动发酵+结构位）用同一份输入各出一批买入候选，记进 `shadow_pred`；夜里按止损 / 目标价模拟离场结算（次日够到触发价才成交、T+1、跳空按开盘、同日两者都碰按止损、一字跌停顺延、第 5 天收盘期满），扣双边滑点与费率。正式台账仍是"持有 5 天看收盘"，两本账的胜率不能直接比。影子盘只评估新开仓，不看持仓
