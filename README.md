...
调度只有进程内这一套：启动网页服务时自动拉起采集守护进程（`scripts/daemon.ts`），网页关了它也继续跑；带 PID 锁，不会起两个。
不往操作系统里装任何定时任务（launchd / schtasks / cron）——那种任务换一台电脑不会跟着走，还会把安装当时的 Node 路径写死。
采集时段（盘前到收盘、盘后两个时点）守护进程自己申请防休眠，时段外放手；合盖（未接电源和外接显示器时）与关机期间任何程序都采不了。
设置页「调度状态」显示守护进程是否在跑、各任务最近一次的结果；它没在跑时标红。
