...
只有一个主页 `/` 作战台，三栏：左栏是盘面环境（环境档位、情绪阶段、盘面温度、影子盘摘要），中栏是选中那只票的决策卡（动作、现价 / 触发 / 止损 / 目标、盈亏比、理由、技术面提示、下一步）和交互式 K 线，右栏是自选列表（今日候选 / 持仓 / 观察池，带迷你走势，点哪只看哪只，↑↓ 切换）。左侧导航可折叠成图标栏（`[` 键），顶栏是快照时点、缺口徽标与通知铃。其余功能都是从右侧滑出的抽屉，数字键直达、Esc 关闭，地址可刷新可收藏：
